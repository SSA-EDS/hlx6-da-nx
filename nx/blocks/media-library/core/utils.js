...
export async function livePreviewLogin(owner, repo) {
  try {
    ...
    debugLog('Setting preview.entmseds-da.live cookie', { owner, repo, url });
    ...
    if (!response.ok) {
      debugLog('Preview.entmseds-da.live login failed', { status: response.status });
      return false;
    }

    debugLog('Preview.entmseds-da.live cookie set successfully');
    return true;
  } catch (error) {
    debugLog('Preview.entmseds-da.live login failed', error);
    return false;
  }
}
