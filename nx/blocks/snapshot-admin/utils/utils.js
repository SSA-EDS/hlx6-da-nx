...
function formatResources(name, resources) {
  return resources.map((res) => ({
    ...
    aemLive: `https://main--${site}--${org}.entmseds.live${res.path}`,
    daEdit: `https://entmseds-da.live/edit#/${org}/${site}${res.path}`,
    daSnapshotEdit: `https://entmseds-da.live/edit#/${org}/${site}/.snapshots/${name}${res.path}`,
  }));
}
...
export async function getUserPublishPermission(path = '/') {
  try {
    // Use the admin.entmseds.page status endpoint to check permissions
    const statusURL = `https://admin.entmseds.page/status/${org}/${site}/main${path}`;
