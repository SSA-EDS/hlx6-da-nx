...
class Form extends LitElement {
  ...
  // TODO(EW): cross-app nav to the entmseds-da.live schema editor. Revisit if EW should
  // own schema authoring rather than linking out.
  _schemaEditorHref() {
    const { owner, repo } = this._details ?? {};
    if (!owner || !repo) return 'https://entmseds-da.live/apps/schema';
    return `https://entmseds-da.live/apps/schema#/${owner}/${repo}`;
  }

  // TODO(EW): cross-app nav back to the entmseds-da.live home. Revisit if EW should
  // provide its own "home" target.
  _goHome() {
    const { owner, repo } = this._details ?? {};
    if (!owner || !repo) return;
    const query = window.location.search ?? '';
    window.location.href = `https://entmseds-da.live${query}#/${owner}/${repo}`;
  }
