...
describe('GLaaS multimodal save', () => {
  it('strips content.entmseds-da.live org/site segments from image URL', () => {
    expect(siteRelativePathFromContentDaLiveUrl(
      'https://content.entmseds-da.live/adobecom/da-dc/acrobat/online/test/.acrobat-pro/report.png',
  ...
  describe('media bus POST path', () => {
    it('postImageToDaMedia hits /media/{org}/{site}/{lang}{site-relative path}', async () => {
      const fetchStub = sinon.stub(window, 'fetch').resolves(new Response(
        JSON.stringify({ uri: 'https://main--da-dc--adobecom.entmseds.page/media_abc.avif' }),
      ...
      expect(result.url).to.equal('https://main--da-dc--adobecom.entmseds.page/media_abc.avif');
    ...
    it('postImageToDaMedia supports nested paths and locale codes with hyphens', async () => {
      const fetchStub = sinon.stub(window, 'fetch').resolves(new Response(
        JSON.stringify({ url: 'https://main--da-dc--adobecom.entmseds.page/media_nested.avif' }),
        { status: 200, headers: { 'Content-Type': 'application/json' } },
      ));
  ...
  it('prepareMultimodalPageForSave posts images to media and rewrites html', async () => {
    ...
    const contentDaLiveUrl = `https://content.entmseds-da.live/${org}/${site}/acrobat/shared/hero.png`;
    const translatedHtml = `<img src="${contentDaLiveUrl}">`;
    const deliveryUrl = 'https://main--da-dc--adobecom.entmseds.page/media_abc.avif';
    ...
    expect(result.text).to.include('main--da-dc--adobecom.entmseds.page/media_abc.avif');
  ...
  it('prepareMultimodalPageForSave skips oversized images and keeps source URLs in html', async () => {
    ...
    const htmlAssetName = '/drafts/page.html';
    const contentDaLiveUrl = `https://content.entmseds-da.live/${org}/${site}/acrobat/shared/hero-large.jpg`;
    const translatedHtml = `<img src="${contentDaLiveUrl}">`;
    const oversized = new Blob([new Uint8Array(MEDIA_IMAGE_UPLOAD_MAX_BYTES + 1)], { type: 'image/jpeg' });
    const warnings = [];
  ...
  it('rewrites img[src] and mirrors delivery URL onto picture source[srcset]', () => {
    ...
    const html = `
      <picture>
        <source srcset="https://content.entmseds-da.live/adobecom/da-dc/acrobat/foo/rect%201.png 1x">
        <source srcset="https://content.entmseds-da.live/adobecom/da-dc/acrobat/foo/rect%201.png 1x" media="(min-width: 600px)">
        <img src="https://content.entmseds-da.live/adobecom/da-dc/acrobat/foo/rect%201.png">
      </picture>
    `;
    ...
    expect(out).not.to.include('content.entmseds-da.live/adobecom/da-dc/acrobat/foo/rect%201.png');
  });

  it('rewrites comma-containing filenames without srcset comma splitting', () => {
    const contentDaLiveUrl = 'https://content.entmseds-da.live/adobecom/da-dc/drafts/demo/.hero/variant=default,%20width=half%20or%20third,%20content=feature%20image.png';
    const deliveryUrl = 'https://main--da-dc--adobecom.entmseds.page/media_hero.avif';
    const html = `
      <picture>
    ...
    expect(out).not.to.include('content.entmseds-da.live');
